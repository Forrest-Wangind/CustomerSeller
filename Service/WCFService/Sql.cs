...
namespace WCFService
{
    public class Sql
    {
        ...
        public static string customerInfoDetail = @" SELECT TOP {0} [CustomerID],RowNumber 序号,[CustomerName] 客户姓名,[CustomerGender] 客户性别,CustomerPhone 客户的电话,[PhoneStratus] 电话的状态, 
                                             [CreateTime] 创建时间,[DealTime] 成交时间,[PhoneType] 电话类型,remark [备注],分配的员工 FROM (
                                             SELECT ROW_NUMBER() OVER (ORDER BY CustomerID) AS RowNumber,[CustomerID],[CustomerName],[CustomerGender],STUFF([CustomerPhone],4,5,'*****') as CustomerPhone,[CustomerAddress],[PhoneStratus],[CreateTime],[DealTime],[PhoneType],substring(remark,1,20) as remark,
