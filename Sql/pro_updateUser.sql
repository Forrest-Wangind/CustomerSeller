...
BEGIN
	...
	BEGIN TRAN tranAdd
		...
		BEGIN
			...
			BEGIN
				UPDATE UserInfo SET UserName = @name,UserGender=@gender,Exten=@exten,EntryTime=@entryTime
				WHERE UserID = @id
				IF(@@error=0)
				BEGIN
					SET @flag = 1;	--更新成功
			...
			ELSE
			BEGIN
				UPDATE UserInfo SET UserName = @name,[Password]=@pass,UserGender=@gender,Exten=@exten,EntryTime=@entryTime
					WHERE UserID = @id
				IF(@@error=0)
				BEGIN
					SET @flag = 1;	--更新成功
